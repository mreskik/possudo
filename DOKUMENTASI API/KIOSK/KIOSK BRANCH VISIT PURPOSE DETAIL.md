...
- `notes_menu` (di tiap `items[]` dan di tiap `menu_package_list[]`) — array string **full_notes** dari `master_notes_menu_detail` yang applies_to-nya match ke `category_id`/`subcategory_id` item itu (lihat "Update (2026-09-24)" di bawah). POS punya field sejenis di `/api/master/menu-list` (`notesMenu`, camelCase, isinya `short_notes`) — lihat `MASTER/NOTES MENU BY ITEM CONV.md`. Array kosong `[]` kalau gak ada notes menu yang match (bukan `null`).
...
`notes_menu` ditambahin di tiap `items[]` (menu utama) **dan** di tiap `menu_package_list[]` (sub-item package) — keputusan bisnis: Kiosk sudah narik seluruh pohon menu sekali di awal (bukan pola on-demand per-klik), jadi notes menu-nya langsung diembed sekalian di sini biar gak perlu roundtrip tambahan pas customer buka form notes. Endpoint terpisah `GET /api/kiosk/notes-menu/{item_conv_id}` yang sempat dibuat buat ini **sudah dihapus** — digantikan penuh oleh field ini.

**Update lanjutan (2026-09-24, hari yang sama)**: POS ikut dirombak pola yang sama — endpoint `GET /api/master/notes-menu/{item_conv_id}` juga **dihapus**, diganti field `notesMenu` (camelCase, isinya `short_notes`) yang di-embed langsung di `/api/master/menu-list` (lihat `MASTER/NOTES MENU BY ITEM CONV.md`). Karena kedua channel sekarang butuh embed, resolusi notes menu dipindah masuk ke `MenuServices::GetMasterMenuList()` itu sendiri (bukan lagi dikerjakan terpisah di `KioskController`) — field `$channel` yang sudah ada (`'pos'`/`'kiosk'`) dipakai juga buat nentuin isi notes-nya `short_notes` atau `full_notes`:

- `MenuServices::GetAllNotesMenuForMatching()` — tarik semua baris notes menu + detail (JOIN tanpa filter) **sekali** di awal `GetMasterMenuList()`, bukan query per item — biar gak N+1 pas loop semua item pohon menu.
- `MenuServices::ResolveNotesMenu($allNotesMenu, $categoryId, $subCategoryId, $field)` — matching in-memory per item/sub-item, `$field` = `'shortNotes'` (channel `pos`) atau `'fullNotes'` (channel `kiosk`). Hasilnya diisi ke `$itemConvRow->notesMenu` (item utama) dan `$mpl->notesMenu` (sub-item package) di dalam `GetMasterMenuList()`.
- `KioskController::mapKioskMenuItem()` tinggal passthrough `$item->notesMenu`/`$mpl->notesMenu` ke `notes_menu` snake_case — gak resolve apa-apa lagi sendiri.

Rule matching-nya sama kayak sebelumnya: `all_category` selalu ikut, `category`/`sub_category` match `category_id`/`subcategory_id`. String duplikat (notes menu yang sama match ke banyak item) di-dedup PER BARIS (key array di `ResolveNotesMenu()`), tapi TETAP terduplikasi ANTAR item (kalau 50 item sama-sama match notes menu `all_category`, string itu ke-copy 50x di response — trade-off yang disengaja demi kesederhanaan struktur, dibanding taruh notes menu terpisah di root dan biar FE yang gabungin).

**Sub-item package** (`menu_package_list[].notes_menu`) matching-nya pakai `category_id`/`subcategory_id` **milik sub-item itu sendiri** (query `$menuPackageDetail` di `MenuServices` ditambah kolom `categoryId`/`subCategoryId` dari `mr_item` sub-item), BUKAN ikut `category_id`/`subcategory_id` item paket induknya — karena 1 package bisa berisi sub-item lintas kategori (mis. minuman di dalam package makanan).

Konsumen frontend (`posv1-vue`, `orderPage.vue`) ikut dirombak — `getNotesMenuByItemConv()` (fetch on-demand) dihapus total dari `MasterServices.ts`, `openEditOrderListDialog()` dan `prefetchNotesMenuForPackage()` sekarang baca langsung dari data yang sudah ada (`itemOrder.notesMenu`/`sub.notesMenu`, dibawa dari `tbMasterDataMenu` pas item di-push ke cart), bukan network call lagi. Build `posv1-vue` lolos tanpa error setelah perubahan ini.
