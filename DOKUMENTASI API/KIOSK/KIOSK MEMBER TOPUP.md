...
1. **Laravel** (`MemberBalanceServices::TopupBalance()`) — resolve `payment_method_id` lokal (`mr_payment_method`) → `payment_gateway_code` **cuma buat fail-fast/validasi UX** (tolak kalau kosong, lihat validasi di atas). `payment_method_id`-nya **SENDIRI** yang diteruskan ke APIANDORDER (2026-09-22, sebelumnya cuma `payment_gateway_code` hasil resolve yang dikirim — lihat catatan perbaikan di bawah). `phone_number` diterusin apa adanya, **bukan** di-resolve ke `member_id` di sini.
2. **APIANDORDER** (`CreateTopup`) — validasi `phone_number`/`amount`/`source`, resolve `member_id` dari `phone_number` (query langsung `master_member`, sama query kayak `member.CheckByPhone()` — Laravel gak pernah pegang `member_id`). `payment_gateway_code` di-**resolve ulang SERVER-SIDE** dari `payment_method_id` (`master_payment_method` pusat) — gak dipercaya dari body Laravel lagi.
...
4. Karena `payment_method_id` udah dipastiin keisi & valid dari langkah 1-2, jalur yang kepake dari Kiosk **selalu gateway**: insert `member_topup_online` (`status='pending'`, `payment_method_id` DAN `payment_gateway_code` snapshot dua-duanya tersimpan), minta QR ke service `payment` (`POST /payment-gateway/qris`, `order_id = reference_number`), simpan `expired_at` dari response. (Service `CreateTopup` di APIANDORDER sendiri generic — masih support jalur tunai langsung kalau `payment_method_id` dikirim kosong, dipakai nanti buat POS; Kiosk gak pernah lewat situ karena validasi Laravel di langkah 1 udah nutup kemungkinan itu.)

**Perbaikan 2026-09-22 (bug ambiguitas akun jurnal)**: sebelumnya `member_topup_online` cuma nyimpen `payment_gateway_code` (string, mis. `"MIDTRANS_QRIS"`) — `payment_method_id` dibuang di langkah Laravel. Ternyata ini bug: `payment_gateway_code` **gak unik** (gak ada constraint di `master_payment_method`), jadi kalau ada >1 payment method beda pakai kode gateway yang sama tapi `coa_accout_id` beda, `memberbalancejurnal` (proses jurnal akuntansi, lihat `MEMBER BALANCE JURNAL.md` di sudocore2) bisa **salah resolve akun COA** pas posting jurnal Debit. Diperbaiki: `payment_method_id` diteruskan APA ADANYA sepanjang rantai (Kiosk → Laravel → APIANDORDER → `member_topup_online.payment_method_id`, migration `221`) — aman diteruskan langsung karena `mr_payment_method.id` (lokal POS) = **SAMA PERSIS** `master_payment_method.id` (pusat), hasil sync PULL yang mempertahankan `id` apa adanya (bukan `auto_increment` lokal terpisah).
