...
try {
    ...
    {
        $sisaSpace = $width - strlen($right);
        $widthForNominal = 10;
        $sisaSpaceNominal = $widthForNominal - strlen($right);

        return str_pad($left, $sisaSpace, " ", STR_PAD_LEFT) . $right . "\n";
        str_pad($left, $sisaSpaceNominal, " ", STR_PAD_LEFT);
    }
    ...
    $print->text(separator("-", $charPerLine));
    $print->text(threeline(4, "Nasi Goreng Nusantara", "20.000", $charPerLine));
